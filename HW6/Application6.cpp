...
#if WITHOUT_TEXTURE 	/* set up app-defined camera if desired, else use camera defaults */
#else
    camera.position[X] = -3;
...
*/
	...
	*/
		...
		interpStyle = GZ_NORMALS;// GZ_COLOR;         /* Phong shading */
...
#if WITHOUT_TEXTURE   /* set up null texture function or valid pointer */
...
int Application6::Render() 
{
	...
	for (int nCnt = 0; nCnt < AAKERNEL_SIZE; nCnt++)
	{
		m_pRenderAlias[nCnt] = *(m_pRender);
		m_pRenderAlias[nCnt].display = (GzDisplay*)malloc(sizeof(GzDisplay));
		status |= GzNewDisplay(&(m_pRenderAlias[nCnt].display), m_nWidth, m_nHeight);
		status |= GzInitDisplay(m_pRenderAlias[nCnt].display);
		...
		*/
		while (fscanf(infile, "%s", dummy) == 1) { 	/* read in tri word */
			...
			//GzPutTriangle(m_pRender, 3, nameListTriangle, valueListTriangle);
				m_pRenderAlias[nCnt].Xoffset = AAFilter[nCnt][0];
				m_pRenderAlias[nCnt].Yoffset = AAFilter[nCnt][1];
				m_pRenderAlias[nCnt].weight = AAFilter[nCnt][2];
			GzPutTriangle(&m_pRenderAlias[nCnt], 3, nameListTriangle, valueListTriangle);
		}

		GzPixel* astGzPixel = m_pDisplay->fbuf;
		GzPixel* astGzPixelTemp = m_pRenderAlias[nCnt].display->fbuf;
		for (int nCntPix = 0; nCntPix < (m_pDisplay->xres*m_pDisplay->yres); nCntPix++)
		{
			if (nCnt == 0)
			{
				astGzPixel->alpha = astGzPixelTemp->alpha;
				astGzPixel->z = astGzPixelTemp->z;
				astGzPixel->blue = m_pRenderAlias[nCnt].weight*astGzPixelTemp->blue;
				astGzPixel->green = m_pRenderAlias[nCnt].weight*astGzPixelTemp->green;
				astGzPixel->red = m_pRenderAlias[nCnt].weight*astGzPixelTemp->red;
			}
			else
			{
				astGzPixel->blue += m_pRenderAlias[nCnt].weight*astGzPixelTemp->blue;
	...
	//m_pDisplay = m_pRenderAlias[0].display;
